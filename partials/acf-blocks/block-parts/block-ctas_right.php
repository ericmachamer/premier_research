...
if ((isset($btn) && is_array($btn)) || is_array($advanced_sections) && in_array('cta', $advanced_sections) || have_rows('calls_to_action_right')) {
    ?>
        <div class="hero-cta-holder">
            <?php
                if(have_rows('calls_to_action_right')) {
                    $count = count(get_field('calls_to_action_right'));
                } else {
            ...
            <div class="row justify-content-center <?php if(get_field('text_alignment') != 'full-text') { ?>justify-content-lg-start<?php } ?> <?php if($count === 1 && get_field('text_alignment') != 'full-text') { ?>justify-content-lg-start <?php } ?>full-btn">
                    ...
                    if(have_rows('calls_to_action_right')) {
                        $count = count(get_field('calls_to_action_right'));
                        while (have_rows('calls_to_action_right')) {
                            the_row()
                            ?>
                            <?php
                            $target = '_self';
                            ...
                            ?>
                            <a class="animate col-12 <?php if(get_field('text_alignment') === 'full-text') { ?><?php if($count === 3) { ?>col-xl-4 col-xxl-3<?php } else { ?>col-lg-6 col-xxl-5<?php } ?><?php } else if(isset($block) && $block['name'] === 'acf/slider') { echo 'col-11'; } else { ?>col-lg-12 col-xl-auto<?php } ?>" href="<?= $url; ?>"
